...
class NetWitnessBackend(TextQueryBackend):
    ...
    field_quote: ClassVar[str | None] = None  # Char used to quote field characters if field_quote_pattern matches
    field_quote_pattern: ClassVar[Pattern[str] | None] = None  # Quote field names if this pattern (doesn't) matches
    field_quote_pattern_negation: ClassVar[bool] = True  # Field name is quoted if pattern doesn't matches when True

    # Values
    str_quote: ClassVar[str] = "'"  # string quoting character (added as escaping character)
    str_quote_pattern: ClassVar[Pattern[str] | None] = None  # re.compile("^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$")
    str_quote_pattern_negation: ClassVar[bool] = True
    escape_char: ClassVar[str | None] = "\\"  # Escaping character for special characters inside string
    wildcard_multi: ClassVar[str | None] = ""  # Character used as multi-character wildcard
    wildcard_single: ClassVar[str | None] = ""  # Character used as single-character wildcard
    ...
    startswith_expression: ClassVar[str | None] = "{field} begins {value}"
    endswith_expression: ClassVar[str | None] = "{field} ends {value}"
    contains_expression: ClassVar[str | None] = "{field} contains {value}"
    ...
    re_expression: ClassVar[str | None] = "{field} regex '{regex}'"
    ...
    re_flags: ClassVar[dict[SigmaRegularExpressionFlag, str]] = {  # type: ignore[misc]
    ...
    case_sensitive_startswith_expression: ClassVar[str | None] = "{field} casematch_startswith {value}"
    case_sensitive_endswith_expression: ClassVar[str | None] = "{field} casematch_endswith {value}"
    case_sensitive_contains_expression: ClassVar[str | None] = "{field} casematch_contains {value}"
    ...
    cidr_expression: ClassVar[str | None] = "{field} = {value}"
    ...
    exists_token: ClassVar[str | None] = "exists"
    not_exists_token: ClassVar[str | None] = "!exists"

    # Null/None expressions
    # Expression for field has null value as format string with {field} placeholder for field name
    field_null_expression: ClassVar[str | None] = f"{{field}} {not_exists_token} || {{field}} = '-'"

    # Field existence condition expressions.
    # Expression for field existence as format string with {field} placeholder for field name
    field_exists_expression: ClassVar[str | None] = f"{{field}} {exists_token}"
    ...
    field_not_exists_expression: ClassVar[str | None] = f"{{field}} {not_exists_token}"
    ...
    field_in_list_expression: ClassVar[str | None] = "{field} {op} {list}"
    ...
    list_separator: ClassVar[str | None] = ","  # List element separator
    ...
    unbound_value_re_expression: ClassVar[str | None] = "_=~{value}"
    ...
    def convert_condition_not(self, cond: ConditionNOT, state: ConversionState) -> str | DeferredQueryExpression:
        ...
        if self.not_token is None or self.group_expression is None:
            msg = "Values for 'not_token' and 'group_expression' are needed to be set"
            raise NotImplementedError(msg)

        if not cond.args:
            msg = "Given condition contains no arguments"
        ...
        try:
            ...
            msg = "Operator 'not' isn't supported by the backend"
    ...
    def convert_condition_field_eq_expansion(
        ...
        if not isinstance(cond.value, SigmaExpansion):
            msg = "Only conditions with SigmaExpansion values are allowed for this method"
            raise TypeError(msg)
    ...
    def decide_convert_condition_as_in_expression(
        ...
        if (not self.convert_or_as_in and isinstance(cond, ConditionOR)) or (
            not self.convert_and_as_in and isinstance(cond, ConditionAND)
        ):
            return False
        ...
        arg_contains_special_chars = any(
        ...
        if not self.in_expressions_allow_wildcards and arg_contains_special_chars:  # noqa: SIM103
            return False
    ...
    def is_contains(self, arg: ConditionFieldEqualsValueExpression | ConditionValueExpression) -> bool:
        ...
        return bool(
            ...
            and len(arg.value.s) == 3  # noqa: PLR2004
    ...
    def is_begins(self, arg: ConditionFieldEqualsValueExpression | ConditionValueExpression) -> bool:
        ...
        return bool(
            isinstance(arg.value, SigmaString) and len(arg.value.s) == 2 and isinstance(arg.value.s[1], SpecialChars)  # noqa: PLR2004
        )
    ...
    def is_ends(self, arg: ConditionFieldEqualsValueExpression | ConditionValueExpression) -> bool:
        ...
        return bool(
            isinstance(arg.value, SigmaString) and len(arg.value.s) == 2 and isinstance(arg.value.s[0], SpecialChars)  # noqa: PLR2004
        )

    def unpack_condition_if_necessary(self, cond: ConditionOR | ConditionAND) -> ConditionOR | ConditionAND:
    ...
    def convert_or_expressions_into_sub_expressions(  # noqa: C901
        ...
        if not self.field_in_list_expression:
            msg = "Value for 'field_in_list_expression' isn't set in the backend"
            raise SigmaBackendError(msg)

        if not self.or_in_operator:
            msg = "Value for 'or_in_operator' isn't set in the backend"
            raise SigmaBackendError(msg)
    ...
    def convert_condition_as_in_expression(
        ...
        msg = "DeferredQueryExpression type is not implemented yet"
        raise NotImplementedError(msg)
